...
$file_name = "./source/lvbo.sql";//数据文件
...
//修改配置文件
				...
				$configStr = str_replace('lvbodb',$dbname,$configStr);
...
//替换前缀这里可以考虑替换哦lvbo_的数据库前缀 但为啦系统的知名度暂时不替换 
//$content=str_replace("lvbo_",TABLE_PRE,$content); 
$carr=array(); 
$iarr=array(); 
...
$admin_sql = "INSERT INTO `lvbo_admin` (`id`, `username`, `password`, `lastlogintime`, `lastloginip`, `status`, `is_client`) VALUES (1, '{$admin_name}', '{$admin_pwd}', 1435742234, '127.0.0.1', 1, 0);";
@mysqli_query($conn,$admin_sql);
if($vip_name != "" && $vip_pwd != ""){
$admin_sql = "INSERT INTO `lvbo_vip_mess` (`vip_name`, `vip_pwd`) VALUES ('{$vip_name}', '{$vip_pwd}');";
@mysqli_query($conn,$admin_sql);
}
