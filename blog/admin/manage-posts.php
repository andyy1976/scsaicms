...
<div class="width-block">
		<div class="pusl-list">
								<div class="joint"><span>你的位置：首页 >  专家页面 > <a href="/blog/admin/index.php">我的文章</a> > 管理文章   <!-- <?php echo "<a href=\"{$menu->addLink}\">" . _t("新增") . "</a>"; ?>--></span></div>
								<div class="bule"></div>
									<div class="main">
									    <div class="body container">
									        
									        <div class="row typecho-page-main" role="main" style="font-size:16px;">
									            <div class="col-mb-12 typecho-list">
									            			
															    <div class="tw-menu">
               									
	                    								<ul class="typecho-option-tabs right">
										                    <?php if($user->pass('editor', true) && !isset($request->id)): ?>
                   									  ...
                   									  <ul class="typecho-option-tabs left">
                       										<li<?php if(!isset($request->status) || 'all' == $request->get('status')): ?> class="current1"<?php endif; ?>><a href="<?php $options->adminUrl('manage-posts.php'
									                        . (isset($request->id) ? '?id=' . $request->id : '')); ?>"><?php _e('可用'); ?></a></li>
									                        <li<?php if('waiting' == $request->get('status')): ?> class="current"<?php endif; ?>><a href="<?php $options->adminUrl('manage-posts.php?status=waiting'
									                        . (isset($request->id) ? '&id=' . $request->id : '')); ?>"><?php _e('待审核'); ?>
									                        <?php if(!$isAllPosts && $stat->myWaitingPostsNum > 0 && !isset($request->id)): ?>
									                            <span class="balloon"><?php $stat->myWaitingPostsNum(); ?></span>
									                        <?php elseif($isAllPosts && $stat->waitingPostsNum > 0 && !isset($request->id)): ?>
									                            <span class="balloon"><?php $stat->waitingPostsNum(); ?></span>
									                        <?php elseif(isset($request->id) && $stat->currentWaitingPostsNum > 0): ?>
									                        ...
									                        . (isset($request->id) ? '&id=' . $request->id : '')); ?>"><?php _e('草稿'); ?>
									                        <?php if(!$isAllPosts && $stat->myDraftPostsNum > 0 && !isset($request->id)): ?>
									                            <span class="balloon"><?php $stat->myDraftPostsNum(); ?></span>
									                        <?php elseif($isAllPosts && $stat->draftPostsNum > 0 && !isset($request->id)): ?>
									                            <span class="balloon"><?php $stat->draftPostsNum(); ?></span>
									                        <?php elseif(isset($request->id) && $stat->currentDraftPostsNum > 0): ?>
									                    ...
									                    <form method="get">
									                        ...
									                        <div class="search" role="search">
									                            ...
									                            <a href="<?php $options->adminUrl('manage-posts.php'
									                                . (isset($request->status) || isset($request->id) ? '?' .
									                                    (isset($request->status) ? 'status=' . htmlspecialchars($request->get('status')) : '') .
									                                    (isset($request->id) ? '?id=' . htmlspecialchars($request->get('id')) : '') : '')); ?>"><?php _e('&laquo; 取消筛选'); ?></a>
									                            <?php endif; ?>
									                            ...
									                            <input type="hidden" value="<?php echo htmlspecialchars($request->get('id')); ?>" name="id" />
									                            <?php endif; ?>
									                ...
									                <div class="typecho-table-wrap">
									                    <table class="typecho-list-table" style="font-size:16px;">
									                        <colgroup>
									                            <col width="20"/>
									                            <col width="6%"/>
									                            <col width="45%"/>
									                        ...
									                        <tbody>
									                            ...
									                            <tr id="<?php $posts->theId(); ?>">
									                                ...
									                                <td><a href="<?php $options->adminUrl('manage-posts.php?id=' . $posts->author->id); ?>"><?php $posts->author(); ?></a></td>
									                                <td><?php $categories = $posts->categories; $length = count($categories); ?>
									                                <?php foreach ($categories as $key => $val): ?>
									                                    <?php echo '<a href="';
									                                    $options->adminUrl('manage-posts.php?category=' . $val['mid']
									                                    . (isset($request->id) ? '&id=' . $request->id : '')
