...
class Widget_Users_Profile extends Widget_Users_Edit implements Widget_Interface_Do
{
    ...
    public function execute()
    {
        ...
        $this->request->setParam('id', $this->user->id);
    }
    ...
    public function updateProfile()
    {
        ...
        /** 取出数据 */
        $user = $this->request->from('email', 'screenName', 'url');
        $user['screenName'] = empty($user['screenName']) ? $user['username'] : $user['screenName'];

        /** 更新数据 */
        $this->update($user, $this->db->sql()->where('id = ?', $this->user->id));

        /** 设置高亮 */
        $this->widget('Widget_Notice')->highlight('user-' . $this->user->id);
    ...
    public function updateOptions()
    {
        ...
        foreach ($settings as $name => $value) {
            ...
            ->from('table.options')->where('name = ? AND user = ?', $name, $this->user->id))->num > 0) {
                $this->widget('Widget_Abstract_Options')
                ->update(array('value' => $value), $this->db->sql()->where('name = ? AND user = ?', $name, $this->user->id));
            } else {
    ...
    public function updatePassword()
    {
        ...
        $this->db->sql()->where('id = ?', $this->user->id));
    ...
    public function updatePersonal()
    {
        ...
        if (!$this->personalConfigHandle($className, $settings)) {
            ...
            ->from('table.options')->where('name = ? AND user = ?', $name, $this->user->id))->num > 0) {
                $this->widget('Widget_Abstract_Options')
                ->update(array('value' => serialize($settings)), $this->db->sql()->where('name = ? AND user = ?', $name, $this->user->id));
            } else {
                $this->widget('Widget_Abstract_Options')->insert(array(
                    'name'  =>  $name,
