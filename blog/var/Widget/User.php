...
class Widget_User extends Typecho_Widget
{
    ...
    public function execute()
    {
        if ($this->hasLogin()) 
        {
            ...
            ->from('table.options')->where('user = ?', $this->_user['id']));
    ...
    public function login($name, $password, $temporarily = false, $expire = 0)
    {
        ...
        ->from('table.member')
        ->where((strpos($name, '@') ? 'email' : 'username') . ' = ?', $name)
        ->limit(1));
        ...
        $hashValidate = $this->pluginHandle()->trigger($hashPluggable)->hashValidate($password, $user['userpwd']);
        if (!$hashPluggable) {
            if ('$P$' == substr($user['userpwd'], 0, 3)) {
                $hasher = new PasswordHash(8, true);
                $hashValidate = $hasher->CheckPassword($password, $user['userpwd']);
            } 
            else 
            {
                $hashValidate = Typecho_Common::hashValidate($password, $user['userpwd']);
            }
        }
        ...
        if ($user && $hashValidate) {
            ...
            if (!$temporarily) 
            {
                ...
                Typecho_Cookie::set('__typecho_id', $user['id'], $expire);
    ...
    /**
     * 只需要提供id即可登录的方法, 多用于插件等特殊场合
     * 
     * @access public
     * @param integer $id 用户id
     * @return boolean
     */
    public function simpleLogin($id)
    {
        $user = $this->db->fetchRow($this->db->select()
        ->from('table.member')
        ->where('id = ?', $id)
    ...
    public function logout()
    {
        ...
        Typecho_Cookie::delete('__typecho_id');
    ...
    public function hasLogin()
    {
        ...
        {
            //$cookieid = Typecho_Cookie::get('__typecho_id');
            $cookieid=isset($_SESSION['userid']) ? $_SESSION['userid'] : NULL;
      
          //  echo  $cookieid ;
            if (NULL != $cookieid) 
            {
                // 验证登陆 
                $user = $this->db->fetchRow($this->db->select()->from('table.member')
                ->where('id = ?', intval($cookieid))
                ->limit(1));
								//echo $user['authCode']."-";
								
                $cookieAuthCode = $_SESSION['cms_userauthCode'];//Typecho_Cookie::get('__typecho_authCode');
               // echo $cookieAuthCode;
