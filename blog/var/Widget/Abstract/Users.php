...
class Widget_Abstract_Users extends Widget_Abstract
{
    ...
    protected function ___options()
    {
        $rows = $this->db->fetchAll($this->db->select()
            ->from('table.options')->where('user = ?', $this->id));
        $options = array();
        foreach ($rows as $row) {
    ...
    public function nameExists($name)
    {
        ...
        if ($this->request->id) {
            $select->where('id <> ?', $this->request->id);
        }
    ...
    {
        $select = $this->db->select(array('COUNT(id)' => 'num'))->from('table.member')
        ->where("table.member.{$column} > {$offset}");
    ...
    {
        return $this->db->fetchObject($condition->select(array('COUNT(id)' => 'num'))->from('table.member'))->num;
    }
    ...
    {
        return $this->db->query($this->db->insert('table.member')->rows($rows));
    }
    ...
    {
        return $this->db->query($condition->update('table.member')->rows($rows));
    }
