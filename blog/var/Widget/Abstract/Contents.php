...
class Widget_Abstract_Contents extends Widget_Abstract
{
    ...
    {
        return $this->widget('Widget_Users_Author@' . $this->cid, array('id' => $this->authorId));
    }
    ...
    public function insert(array $content)
    {
        ...
        $insertStruct = array(
            ...
            'authorId'      =>  isset($content['authorId']) ? $content['authorId'] : $this->user->id,
    ...
    public function isWriteable(Typecho_Db_Query $condition)
    {
        ...
        return $post && ($this->user->pass('editor', true) || $post['authorId'] == $this->user->id);
    }
    ...
    public function allow()
    {
        ...
        foreach ($permissions as $permission) {
            ...
            if ('edit' == $permission) {
                $allow &= ($this->user->pass('editor', true) || $this->authorId == $this->user->id);
            } else {
                /** 对自动关闭反馈功能的支持 */
