...
class Widget_Abstract_Comments extends Widget_Abstract
{
    ...
    {
        return $this->db->select('table.comments.coid', 'table.comments.cid', 'table.comments.author', 'table.comments.email', 'table.comments.url', 'table.comments.ip',
    ...
    public function insert(array $comment)
    {
        ...
        $insertStruct = array(
            ...
            'email'      =>  empty($comment['email']) ? NULL : $comment['email'],
    ...
    {
        if (empty($condition)) {
            if ($this->have() && ($this->user->pass('editor', true) || $this->ownerId == $this->user->id)) {
                return true;
            }
        } else {
            $post = $this->db->fetchRow($condition->select('ownerId')->from('table.comments')->limit(1));

            if ($post && ($this->user->pass('editor', true) || $post['ownerId'] == $this->user->id)) {
                return true;
            }
        }
    ...
    public function author($autoLink = NULL, $noFollow = NULL)
    {
        ...
        if ($this->url && $autoLink) 
        {
            ...
            echo '<a href="' , "/index.php?s=memberlist/user&userid=". $this->authorId, '"' , ($noFollow ? ' rel="external nofollow"' : NULL) , '>' , $this->author , '</a>';
        } 
        else 
        {
            echo $this->author;
    ...
    public function gravatar($size = 32, $default = NULL)
    {
        if ($this->options->commentsAvatar && 'comment' == $this->type) {
            ...
            if (!$plugged) {
                $url = Typecho_Common::gravatarUrl($this->email, $size, $rating, $default, $this->request->isSecure());
