...
class Widget_Ajax extends Widget_Abstract_Options implements Widget_Interface_Do
{
    ...
    public function feed()
    {
        ...
        if ($client) {
            ...
            /** 匹配内容体 */
            $response = $client->getResponseBody();
            preg_match_all("/<item>\s*<title>([^>]*)<\/title>\s*<link>([^>]*)<\/link>\s*<gid>[^>]*<\/gid>\s*<pubDate>([^>]*)<\/pubDate>/is", $response, $matches);
    ...
    public function editorResize()
    {
        ...
        ->from('table.options')->where('name = ? AND user = ?', 'editorSize', $this->user->id))->num > 0) {
            $this->widget('Widget_Abstract_Options')
            ->update(array('value' => $this->request->size), $this->db->sql()->where('name = ? AND user = ?', 'editorSize', $this->user->id));
        } else {
