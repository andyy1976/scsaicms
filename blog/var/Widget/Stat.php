...
/**
     ...
     * @return integer
     */
    protected function ___publishedPostsNumtoday()
    {
       ...
       $day = strtotime(date('Y-m-d', time()))+86400;//86400为一天的秒数
        ...
        return $this->db->fetchObject($this->db->select(array('COUNT(cid)' => 'num'))
                    ...
                    ->where('table.contents.status = ?', 'publish')
                     ->where('table.contents.created >?', $night)
                     ->where('table.contents.created <?', $day))->num;
    }
    /**
     * 获取待审核的文章数目
     *
    ...
    protected function ___myPublishedPostsNum()
    {
        return $this->db->fetchObject($this->db->select(array('COUNT(cid)' => 'num'))
                    ...
                    ->where('table.contents.type = ?', 'post')
                    ->where('table.contents.status = ?', 'publish')
                    ->where('table.contents.authorId = ?', $this->user->id))->num;
    }
    ...
    protected function ___currentPublishedPostsNum()
    {
        return $this->db->fetchObject($this->db->select(array('COUNT(cid)' => 'num'))
                    ...
                    ->where('table.contents.type = ?', 'post')
                    ->where('table.contents.status = ?', 'publish')
                    ->where('table.contents.authorId = ?', $this->request->filter('int')->id))->num;
    }
    ...
    protected function ___myPublishedCommentsNum()
    {
        return $this->db->fetchObject($this->db->select(array('COUNT(coid)' => 'num'))
                    ...
                    ->where('table.comments.ownerId = ?', $this->user->id))->num;
    }
