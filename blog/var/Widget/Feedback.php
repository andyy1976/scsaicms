...
class Widget_Feedback extends Widget_Abstract_Comments implements Widget_Interface_Do
{
    ...
    private function comment()
    {
        ...
        $comment = array(
            ...
            'ownerId'   =>  $this->_content->author->id,
            'type'      =>  'comment',
        ...
        if ($this->options->commentsRequireMail && !$this->user->hasLogin()) {
            $validator->addRule('email', 'required', _t('必须填写电子邮箱地址'));
        }

        $validator->addRule('email', 'email', _t('邮箱地址不合法'));
        ...
        if (!$this->user->hasLogin()) {
            ...
            $comment['email'] = $this->request->filter('trim')->email;
            ...
            Typecho_Cookie::set('__typecho_remember_mail', $comment['email'], $expire);
        ...
        if (!$this->options->commentsRequireModeration && $this->options->commentsWhitelist) {
            if ($this->size($this->select()->where('author = ? AND email = ? AND status = ?', $comment['author'], $comment['email'], 'approved'))) {
    ...
    public function requireUserLogin($userName)
    {
        if ($this->user->hasLogin() && $this->user->screenName != $userName) {
            /** 当前用户名与提交者不匹配 */
            return false;
        } else if (!$this->user->hasLogin() && $this->db->fetchRow($this->db->select('id')
        ->from('table.member')->where('screenName = ? OR username = ?', $userName, $userName)->limit(1))) {
            /** 此用户名已经被注册 */
            return false;
        }
    ...
    public function action()
    {
        ...
        in_array($callback, array('comment', 'trackback'))) {
            ...
            if ('comment' == $callback) {
                ...
                /** 检查ip评论间隔 */
                if (!$this->user->pass('editor', true) && $this->_content->authorId != $this->user->id &&
