...
class Widget_Menu extends Typecho_Widget
{
    ...
    public function execute()
    {
        ...
        array(
            ...
            array(_t('插件'), _t('插件管理'), 'plugins.php', 'administrator',true),
            ...
            array(_t('备份'), _t('备份'), 'backup.php', 'administrator',true),
        ...
        array(
            ...
            array(array('Widget_Contents_Post_Admin', 'getMenuTitle'), array('Widget_Contents_Post_Admin', 'getMenuTitle'), 'manage-posts.php?id=', 'contributor', true),
          //  array(_t('独立页面'), _t('管理独立页面'), 'manage-pages.php', 'editor', false, 'write-page.php'),
            ...
            array(_t('用户'), _t('管理用户'), 'manage-users.php', 'administrator', true, 'user.php'),
            array(_t('新增用户'), _t('新增用户'), 'user.php', 'administrator', true),
            array(array('Widget_Users_Edit', 'getMenuTitle'), array('Widget_Users_Edit', 'getMenuTitle'), 'user.php?id=', 'administrator', true),
        ),
        array(
    ...
    public function output($class = 'focus', $childClass = 'focus')
    {
        foreach ($this->_menu as $key => $node) 
        {
           ...
           // echo "<ul " . ($key == $this->_currentParent ? ' ' . $class : NULL) 
            //    . "><li><span class=\"file\"> <a href=\"{$node[2]}\">{$node[0]}</a></span>"
             //   . "</li><u/l>";
    echo "<ul class=\"root" . ($key == $this->_currentParent ? ' ' . $class : NULL) 
            ...
            foreach ($node[3] as $inKey => $inNode) 
            {
               ...
               // echo "<li><span class=\"arrow\"></span><span class=\"file\"> <a href=\"" . ($key == $this->_currentParent && $inKey == $this->_currentChild ? $this->_currentUrl : $inNode[2]) . "\">{$inNode[0]}</a></span></li>";
            }
