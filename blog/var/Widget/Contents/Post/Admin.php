...
class Widget_Contents_Post_Admin extends Widget_Abstract_Contents
{
    ...
    public function getMenuTitle()
    {
        if (isset($this->request->id)) {
            return _t('%s的文章', $this->db->fetchObject($this->db->select('screenName')->from('table.member')
                ->where('id = ?', $this->request->filter('int')->id))->screenName);
        }
    ...
    public function execute()
    {
        ...
        if (!$this->user->pass('editor', true)) {
            $select->where('table.contents.authorId = ?', $this->user->id);
        } else {
            if ('on' == $this->request->__typecho_all_posts) {
                ...
                if ('on' != Typecho_Cookie::get('__typecho_all_posts')) {
                    $select->where('table.contents.authorId = ?', isset($this->request->id) ?
                        $this->request->filter('int')->id : $this->user->id);
                }
            }
        }
