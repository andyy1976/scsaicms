...
class Typecho_Widget_Helper_PageNavigator_Box extends Typecho_Widget_Helper_PageNavigator
{
    ...
    { 
        ...
        {
            $page= $itemPrevBegin . sprintf($linkPrevBegin,
        ...
        if ($from > 1) 
        {
             $page.= $itemBegin . sprintf($linkBegin, str_replace($this->_pageHolder, 1, $this->_pageTemplate) . $this->_anchor)
                . '1' . $linkEnd . $itemEnd;

            if ($from > 2) 
            {
                //输出省略号
                $page.=  $itemBegin . $textBegin . $splitWord . $textEnd . $itemEnd;
            }
        }
        ...
        for ($i = $from; $i <= $to; $i ++) 
        {
            ...
            $page.=  ($current ? $itemCurrentBegin : $itemBegin) . sprintf(($current ? $linkCurrentBegin : $linkBegin),
        ...
        if ($to < $this->_totalPage) 
        {
            ...
            $page.=  $itemBegin . sprintf($linkBegin, str_replace($this->_pageHolder, $this->_totalPage, $this->_pageTemplate) . $this->_anchor)
        ...
        {
            $page.=  $itemNextBegin . sprintf($linkNextBegin,
                str_replace($this->_pageHolder, $this->_currentPage + 1, $this->_pageTemplate) . $this->_anchor)
