...
function cms_encrypt($data, $key = 'echounion')
{
    $key = md5($key);
    $x = 0;
    $len = strlen($data);
...
function cms_decrypt($data, $key = 'echounion')
{
    $key = md5($key);
    $x = 0;
...
function send_mail($sendto_email, $user_name, $subject, $bodyurl, $port = 25)
{
    ...
    $mail->Password = cms_decrypt(C('MAIL_PASSSWORD')); // SMTP password
...
function remove_html_xss($string){
	...
	require_once './Core/Extend/Vendor/Htmlpurifie/HTMLPurifier.auto.php';
    // 生成配置对象
    ...
    $remote_img = $u[2];
      if(strtolower(substr($remote_img,0,4)) == 'http'){
        if(strlen($remote_img)>4) {
           ...
           // $fn = 'http://www.qiruanwang.com'.$fn;//这里可以加您自己域名http://www.qiruanwang.com
