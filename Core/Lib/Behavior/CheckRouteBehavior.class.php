...
class CheckRouteBehavior extends Behavior {
    ...
    private function parseRule($rule,$route,$regx) {
        ...
        if(0=== strpos($url,'/') || 0===strpos($url,'http')) { // 路由重定向跳转
            ...
            if($paths) {
				...
				preg_replace_callback('@(\w+)\/([^\/]+)@i', function($r){return $var[strtolower($r[1])]=strip_tags($r[2]);}, implode('/',$paths));
            }
            // 解析路由自动传入参数
    ...
    private function parseRegex($matches,$route,$regx) {
        ...
        $url   =  preg_replace_callback('/:(\d+)/',function($match){return $matches[($match[1])];},$url);
		...
		if(0=== strpos($url,'/') || 0===strpos($url,'http')) { // 路由重定向跳转
            ...
            if($regx) {
				...
				preg_replace_callback('@(\w+)\/([^,\/]+)@i',function($r){return $var[strtolower($r[1])]=strip_tags($r[2]);}, $regx);
            }
            // 解析路由自动传入参数
            if(is_array($route) && isset($route[1])) {
                parse_str($route[1],$params);
