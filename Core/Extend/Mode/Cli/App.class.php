        ...
        if(C('URL_MODEL')==1) {// PATHINFO 模式URL下面 采用 index.php module/action/id/4
            ...
            if(count($params)>1) {
                ...
                preg_replace('@(\w+),([^,\/]+)@e', '$_GET[\'\\1\']="\\2";', implode(',',$params));
				 ...
				 preg_replace_callback('@(\w+),([^,\/]+)@i', function($r){ return $_GET[$r[1]]=$r[2]; }, implode(',',$params));	
				}
            }
            ...
            if($_SERVER['argc']>3) {
                ...
                //preg_replace('@(\w+),([^,\/]+)@e', '$_GET[\'\\1\']="\\2";', implode(',',array_slice($_SERVER['argv'],3)));
				 preg_replace_callback('@(\w+),([^,\/]+)@i', function($r){ return $_GET[$r[1]]= $r[2];}, implode(',',array_slice($_SERVER['argv'],3)));					
            }
        }
