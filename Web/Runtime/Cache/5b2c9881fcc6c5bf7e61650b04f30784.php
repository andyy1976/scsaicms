...
<?php $_cur_typeid = $article['typeid']; $_cur_fid = $article['fid']; $_cur_typename = $article['typename']; ?>

<!-- Banner区 -->
<div class="page-banner">
    <div class="banner-overlay"></div>
    <div class="banner-content">
        <h1 class="banner-title"><?php echo (($_cur_typename)?($_cur_typename):'文章详情'); ?></h1>
        <p class="banner-subtitle">深入了解AI驱动的内容生态</p>
    </div>
</div>
...
<div class="breadcrumb-bar">
    <div class="container">
        <i class="fa fa-home"></i> <?php echo ($nav); ?>
    </div>
</div>
    ...
    <div class="container">
        <div class="article-layout">
            ...
            <main class="article-main">
                ...
                <header class="article-header">
                    ...
                    <div class="article-meta">
                        <span class="meta-item">
                            ...
                            <?php echo (date('Y-m-d H:i',$article["addtime"])); ?>
                        </span>
                        <?php if($article['author'] != ''): ?><span class="meta-item">
                                <i class="fa fa-user"></i>
                                <?php echo ($article["author"]); ?>
                            </span><?php endif; ?>
                        <span class="meta-item">
                            <i class="fa fa-eye"></i>
                            <?php echo (($article["hits"])?($article["hits"]):0); ?> 阅读
                        </span>
                        <?php if($article['source'] != ''): ?><span class="meta-item">
                ...
                <!-- 封面图 -->
                <?php if($article['imgurl'] != ''): ?><div class="article-cover">
                        <img src="<?php echo ($article["imgurl"]); ?>" alt="<?php echo ($article["title"]); ?>" onerror="this.onerror=null;this.src='__ROOT__/uploads/images/202504/21/c2banner.jpg';">
                    </div><?php endif; ?>
                ...
                <footer class="article-footer">
                    ...
                    <!-- 标签 -->
                    <?php if($article['tags'] != ''): ?><div class="article-tags">
                            <i class="fa fa-tags"></i>
                            <?php if(is_array($article["tags_arr"])): $i = 0; $__LIST__ = $article["tags_arr"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tag): $mod = ($i % 2 );++$i;?><a href="__ROOT__/index.php?s=Search/index/keyword/<?php echo ($tag); ?>" class="tag-item"><?php echo ($tag); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
                        </div><?php endif; ?>

                    <!-- 来源链接 -->
                    <?php if($article['source_url'] != ''): ?><div class="article-source">
                            <strong>原文链接：</strong>
                            <a href="<?php echo ($article["source_url"]); ?>" target="_blank" rel="nofollow"><?php echo (($article["copyfrom"])?($article["copyfrom"]):'查看原文'); ?></a>
                        </div><?php endif; ?>

                    <!-- 上下篇导航 -->
                    <div class="article-nav">
                        <div class="nav-prev">
                            <label><i class="fa fa-chevron-left"></i> 上一篇</label>
                            <?php if($prevtitle != ''): ?><a href="<?php echo ($prevurl); ?>"><?php echo (msubstr($prevtitle,0,30)); ?></a>
                            <?php else: ?>
                                <span class="nav-none">已是第一篇</span><?php endif; ?>
                        </div>
                        <div class="nav-next">
                            <label>下一篇 <i class="fa fa-chevron-right"></i></label>
                            <?php if($nexttitle != ''): ?><a href="<?php echo ($nexturl); ?>"><?php echo (msubstr($nexttitle,0,30)); ?></a>
                            <?php else: ?>
                                <span class="nav-none">已是最后一篇</span><?php endif; ?>
                        </div>
                    </div>

                    <!-- 返回按钮 -->
                    <div class="article-back">
                        <a href="__ROOT__/index.php?s=Lists/index/typeid/<?php echo ($_cur_typeid); ?>" class="btn-back">
            ...
            <aside class="article-sidebar">
                ...
                <div class="sidebar-widget">
                    <h3 class="widget-title">
                        ...
                        <?php echo ($_cur_typename); ?> - 导航
                    </h3>
                    <ul class="widget-categories">
                        <?php $result=M('type')->where("fid=<?php echo ($_cur_typeid); ?> and 1=1")->order("drank asc")->select();if(is_array($result)): $i = 0;foreach($result as $key=>$vo):++$i;?><li <?php if($vo['typeid'] == $_cur_typeid): ?>class="active"<?php endif; ?>>
                                <a href='<?php echo (url(lists,$vo["typeid"])); ?>'><?php echo ($vo["typename"]); ?></a>
                ...
                <div class="sidebar-widget">
                    ...
                    <ul class="widget-posts">
                        <?php $_recs = M('article')->where("status=1 AND typeid=<?php echo ($_cur_typeid); ?>")->order('hits DESC, addtime DESC')->limit(8)->select(); ?>
                        <?php if(is_array($_recs)): $i = 0; $__LIST__ = $_recs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rec): $mod = ($i % 2 );++$i;?><li>
                                <a href="/index.php?s=/articles/<?php echo ($rec["aid"]); ?>.html" title="<?php echo ($rec["title"]); ?>">
                                    <span class="post-rank"><?php echo ($i); ?></span>
                                    <?php echo (msubstr($rec["title"],0,22)); ?>
                                </a>
                                <span class="post-hits"><i class="fa fa-eye"></i> <?php echo ($rec["hits"]); ?></span>
                ...
                <div class="sidebar-widget">
                    ...
                    <ul class="widget-posts">
                        <?php $_news = M('article')->where('status=1')->order('addtime DESC')->limit(6)->select(); ?>
                        <?php if(is_array($_news)): $i = 0; $__LIST__ = $_news;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nw): $mod = ($i % 2 );++$i;?><li>
                                <a href="/index.php?s=/articles/<?php echo ($nw["aid"]); ?>.html" title="<?php echo ($nw["title"]); ?>">
                                    <?php echo (msubstr($nw["title"],0,22)); ?>
                                </a>
                                <span class="post-date"><?php echo (date('m-d',$_nw["addtime"])); ?></span>
                            </li><?php endforeach; endif; else: echo "" ;endif; ?>
                    </ul>
                </div>
