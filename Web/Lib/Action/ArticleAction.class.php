...
class ArticleAction extends BaseAction
{
    ...
    public function index()
    {
        ...
        if ($config['isread'] == 1) 
        {
            $uvail = explode(',', $_SESSION['cms_uservail']);
            if (!in_array($alist['typeid'], $uvail)) 
            {
                alert('对不起您没有阅读该文章的权限！', __APP__);
            }
        }
        ...
        $rows = $article->where($map)->field('aid,title')->order('istop desc,addtime desc')->select();
        $arr_aid = array();
        foreach ($rows as $row) 
        {
        ...
        if (isset($arr_aid[$cur_key + 1])) {
            $up = $article->where('aid=' . intval($arr_aid[$cur_key + 1]))->find();
        ...
        if (isset($arr_aid[$cur_key - 1])) 
        {
            $down = $article->where('aid=' . intval($arr_aid[$cur_key - 1]))->find();
