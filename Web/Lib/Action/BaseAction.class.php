    ...
    function _initialize()
    {
        ...
        if ($_POST) 
        {
            array_walk_recursive($_POST,'inject_check');
				...
				//获取访客权限
        if (intval(S('first_look')) != 1 || !isset($_SESSION['userid'])) 
        {
            $config = F('basic', '', './Web/Conf/');
            if ($config['isread'] == 1) 
            {
                $group_vail = M('member_group')->where('group_id=' . $config['defaultup'])->getField('group_vail');
                $_SESSION['cms_uservail'] = $group_vail;
            }
            S('first_look', 1);
        }
        
        $refer = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];//$_SERVER['HTTP_REFERER'];
       ...
       // alert($lasturl,1);
        $article = D('ArticleView');
			   $where['status'] = 1;
				 ...
				 $counttotal = $article->where($where)->count();
				 $this->assign('counttotal',$counttotal);
				   
				
				 $night = strtotime(date('Y-m-d', time()));
         $day = strtotime(date('Y-m-d', time()))+86400;//86400为一天的秒数
         $where['addtime'] = ['between',$night.",".$day];//用between判断


				 $counttoday = $article->where($where)->count();
				 $this->assign('counttoday',$counttoday);
				 
				unset($article, $counttotal, $counttoday, $where, $day);
        R('Public/head');
    }
