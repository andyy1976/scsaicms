...
//RSS订阅
    ...
    {
        import('ORG.Util.Rss');
        $myRss = new RSS("律伯人力资源库", C('SERVER_URL'), "律伯人力资源库");
...
//自动更新描述
    ...
    function wxnotify()
    {
		...
		//支付成功业务逻辑
    private  function payed_worker($third_trade_no,$out_trade_no,$total_fee)
    {
        ...
        $tlog = M('trade_log');
        $tdail = $tlog->where("taobao_no='{$third_trade_no}'")->find();
        if (!$tdail) {
            ...
            if ($trade_type == "CZ") {
                    ...
                    if ($uid > 0) {
                        ...
                        //logResult(M('member',null)->getLastSql().'<BR>');
                        $data['uid'] = $uid;
                        $data['addtime'] = time();
