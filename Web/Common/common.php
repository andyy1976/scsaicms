 ...
 */
	...
	* @param int $typeid ：栏目ID
	...
	function show_field($type,$cid,$name,$value="",$option="",$url="")
	{
			if($type=="wenda")
			{
					$model = D('Wenda');
					 ...
					 // if($count == 1)
						{	
						    ...
						    $count = $model->where($map)->count();
								echo "<li value=\"".$temp[1]."\" $select><a href=\"".$url.$name."=".$temp[1]."\">".$temp[0]."(".$count.")</a></li>";
							}
							else if(count($temp) == 1)
							{
								...
								$map[$name] = $temp[0];      
						    $count = $model->where($map)->count();
								echo "<li value=\"".$temp[0]."\" $select><a href=\"".$url.$name."=".$temp[1]."\">".$temp[0]."(".$count.")</a></li>";
							}
						}
						/*
						else if($count > 1)
						{
								...
								if(count($temp) == 2)
								{
									...
									echo "<li value=\"".$temp[1]."\" $select>".$temp[0];
								}
								else if(count($temp) == 1)
								{
									if($temp[0] == $value)
									...
									echo "<li value=\"".$temp[0]."\" $select>".$temp[0];
								}
								...
								for($i=1;$i<$count;$i++)
								{
										...
										if(count($temp) == 2)
										{
											...
											echo "<li value=\"".$temp[1]."\" $select>".$temp[0]."</li>";
										}
										else if(count($temp) == 1)
										{
											if($temp[0] == $value)
											...
											echo "<li value=\"".$temp[0]."\" $select>".$temp[0]."</li>";
										}
		...
		**/
		function url($model,$id)
		{
			if($model=='lists'){
				...
				$url = M('type')->where('typeid='.intval($id) . ' and islink=1')->getField('url');
				if($url != ''){
					S('url_'.$id,$url);
					return $url;
				}
