<?php
	...
	{ 
		...
		//替换采集等通过url参数传值
		function url_replace($xmlurl,$order='asc')
		{
		if($order=='asc')
			{
	...
	function show_field($type,$name,$value="",$option="",$css="")
	{
		...
		case 3://单选下拉列表
			if($option != '')
			{
				...
				if(preg_match("/<php>(.*)</php>/i", $option, $temp))
				{
					eval($temp[1]);
				}
				else
