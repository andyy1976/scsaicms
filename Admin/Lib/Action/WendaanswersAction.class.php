...
class WendaanswersAction extends CommonAction
{	
    public function index()
    {
			...
			$p->setConfig('header','条解答');
			$p->setConfig('first','首 页');
			$p->setConfig('last','末 页');
			$p->setConfig('next','下一页');
			$p->setConfig('theme',"%first%%upPage%%linkPage%%downPage%%end%
			<li><span><select name='select' onChange='javascript:window.location.href=(this.options[this.selectedIndex].value);'>%allPage%</select></span></li>\n<li><span>共<font color='#009900'><b>%totalRow%</b></font>条解答 20条/每页</span></li>");
		...
		public function doedit()
    {
		...
		//使用stripslashes 反转义,防止服务器开启自动转义
			...
			if($pl->save($data))
			{
				$this->assign("jumpUrl",U('Wendaanswers/index'));
				$this->success('操作成功!');
			}
			$this->error('操作失败!');
    }
