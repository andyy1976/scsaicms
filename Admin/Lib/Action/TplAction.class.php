...
{	
     Public function _empty()
		...
		public function index()
		{
			...
			{
				$list_dir[$key]['pathfile'] = url_replace($value['path'],'desc').'|'.$value['filename'];
			}
			$_SESSION['tpl_jumpurl'] = '?s=Tpl/index/id/'.url_replace($dirpath,'desc');
			if($dirlast && $dirlast != '.')
			{
				$this->assign('dirlast',url_replace($dirlast,'desc'));
			}
		...
		public function add()
		{
			$filename = url_replace(str_replace('*','.',trim($_GET['id'])));
			if (empty($filename)) 
			{
				$this->error('模板名称不能为空！');
			}
	  ...
	  public function del()
		{
			$id = url_replace(str_replace('*','.',trim($_GET['id'])));
