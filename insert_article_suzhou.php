...
// 检查是否已存在相同标题的文章
$check_sql = "SELECT aid FROM {$db_prefix}article WHERE title = ?";
...
// 插入文章
$insert_sql = "INSERT INTO {$db_prefix}article 
...
$stmt->bind_param('isssiiisssss', 
    $typeid, $title, $content, $addtime, $updatetime, 
    $status, $hits, $author, $source, $copyfrom, $keywords, $description
);
...
if ($stmt->execute()) {
    ...
    echo "文章ID: $aid\n";
    echo "标题: $title\n";
    echo "栏目ID: $typeid (案例栏目)\n";
    echo "访问链接: /index.php?s=/articles/$aid.html\n";
} else {
